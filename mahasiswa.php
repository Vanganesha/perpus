...
$l=$_SESSION[idmahasiswa];
$cs=  query("SELECT * FROM mahasiswa WHERE nama IS NOT NULL AND nim IS NOT NULL AND alamat IS NOT NULL AND nohp IS NOT NULL AND tempat_lahir IS NOT NULL AND tanggal_lahir IS NOT NULL AND foto IS NOT NULL AND nama_ayah IS NOT NULL AND idmahasiswa=$id");
$ci= mysql_num_rows($cs);
if ($ci==0){
    header('location:editprofil.php?ux=Mahasiswa&page=complete');
}  else {
    $page = 'dashboard_mahasisiwa.vanray';
...
<html 
    ...
    include 'nav.php';
    $q= query("select * from mahasiswa where idmahasiswa=$id");
